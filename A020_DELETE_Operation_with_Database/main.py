# =========================================================
# A020 — DELETE Operation with Database
# =========================================================
# Walks the full SQLAlchemy CRUD pattern:
#   - GET    /todos              → list all rows
#   - GET    /todos/{todo_id}    → read a single row (404 if missing)
#   - POST   /todos              → create a new row
#   - PUT    /todos/{todo_id}    → update an existing row (404 if missing)
#   - DELETE /todos/{todo_id}    → delete a row (404 if missing)
# =========================================================
...
# DELETE
# DELETE /todos/{todo_id}  → delete an existing row
# 1. Look up the row by id
# 2. If not found, raise 404
# 3. `db.delete(todo)` stages the DELETE
# 4. `db.commit()` fires the DELETE statement
...
def delete_todo(todo_id:int, db:Session = Depends(get_db)):
    ...
    # Stage the DELETE — SQLAlchemy marks the object as deleted in the session
    db.delete(todo)
